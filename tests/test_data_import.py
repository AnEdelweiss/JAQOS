...
class TestDataImport:
    ...
    def test_create_factor_success(self, MockFactorsApi, MockExpApi, mock_read_excel):
        ...
        mock_factor_instance.search_factors.side_effect = [
            {"result": []},
            {"result": [mock_factor_result]},
        ]
        ...
        mock_factor_instance.get_factor_levels.return_value = {
            "result": [mock_lvl_result]
        }
    ...
    def test_get_provenances_with_missing_and_default_creation(
        self, mock_basename, MockDataApi, mock_read_excel
    ):
        ...
        mock_data_api.search_provenance.side_effect = [
            {"result": []},
            {"result": [mock_prov]},
        ]
    ...
    @patch("simple.data_import.pd.read_excel")
    @patch("simple.data_import.os.path.basename")
    def test_create_data_success_upload(
        self,
        mock_basename,
        mock_read_excel,
        MockExpApi,
        ...
        mock_data_api_instance.get_data_file_descriptions_by_search.return_value = {
            "result": []
        }
        ...
        sci_obj_uri = {
            "Plant_1": "http://fake-uri.com/so/1",
            "Plant_2": "http://fake-uri.com/so/2",
        }
    ...
    @patch("simple.data_import.pd.read_excel")
    @patch("simple.data_import.os.path.basename")
    def test_create_data_skips_existing(
        self,
        mock_basename,
        mock_read_excel,
        MockExpApi,
        ...
        mock_existing_data._date = (
            fake_time.tz_localize("UTC")
            .tz_convert("UTC")
        ...
        mock_data_api_instance = MockDataApi.return_value
        mock_data_api_instance.search_data_list.return_value = {
            "result": [mock_existing_data]
        }
        mock_data_api_instance.get_data_file_descriptions_by_search.return_value = {
            "result": []
        }
    ...
    @patch("simple.data_import.silex.ExperimentsApi")
    @patch("simple.data_import.pd.read_excel")
    def test_create_sci_obj_success(
        self,
        mock_read_excel,
        MockExpApi,
        MockOntologyApi,
        ...
        df_factors = pd.DataFrame({"name": ["Factor1"], "description": ["Desc 1"]})
        ...
        MockExpApi.return_value.get_available_factors.return_value = {
            "result": [mock_factor_result]
        }
        ...
        MockOntologyApi.return_value.search_sub_classes_of.return_value = {
            "result": [mock_ontology]
        }

        # Objets scientifiques
        mock_sci_obj_api_instance = MockSciObjApi.return_value
        mock_sci_obj_api_instance.search_scientific_objects.return_value = {
            "result": []
        }
        mock_sci_obj_api_instance.create_scientific_object.return_value = {
            "result": ["http://fake-base.com/so/new"]
        }

        # Germplasmes
        mock_germ = MagicMock()
        mock_germ.uri = "http://fake-uri.com/germplasm/1"
        MockGermplasmApi.return_value.search_germplasm.return_value = {
            "result": [mock_germ]
        }

        # 2. Act
        sci_obj_uri, created_count = create_sci_obj(
            "dummy_data.xlsx", "dummy_miappe.xlsx", fake_client
        )
    ...
    @patch("simple.data_import.pd.read_excel")
    @patch("simple.data_import.get_name_space")
    def test_create_sci_obj_skips_existing(
        self,
        mock_get_name_space,
        mock_read_excel,
        MockExpApi,
        ...
        df_data = pd.DataFrame(
            {"Plant ID": ["Plant_1"], "Germplasm": ["Apache"], "Factor1": [None]}
        )
        df_factors = pd.DataFrame({"name": ["Factor1"], "description": ["Desc 1"]})
    ...
    @patch("simple.data_import.silex.ExperimentsApi")
    @patch("simple.data_import.pd.read_excel")
    def test_create_sci_obj_missing_experiment_raises_error(
        self, mock_read_excel, MockExpApi
    ):
        """Teste qu'une erreur est levée si le nom de l'expérience n'existe pas"""
        df_miappe = pd.DataFrame(
            {"name": ["Exp_Inconnue"], "scientific_object_type": ["vocabulary:Plant"]}
        )
