...
class TestExperiment:
    ...
    def test_api_find_experiment_by_name(self, mock_exp_api_class):
        ...
        mock_api_instance.search_experiments.return_value = {"result": [{"uri": "http://test-uri/exp1"}]}
        ...
        mock_api_instance.search_experiments.assert_called_once_with(name="Mon_Experience")
    ...
    def test_create_experiment_already_exists(self, mock_proj_api, mock_sec_api, mock_org_api, mock_exp_api_class, mock_read_excel):
        ...
        result_uris = create_experiment("fake_miappe.xlsx", "fake_client", status_callback=mock_callback)
    ...
    @patch('simple.experiment.silex.SecurityApi')
    @patch('simple.experiment.silex.ProjectsApi')
    def test_create_experiment_success(self, mock_proj_api, mock_sec_api, mock_org_api, mock_exp_api_class, mock_read_excel):
        ...
        mock_exp_instance.search_experiments.side_effect = [{"result": []}, {"result": [faux_resultat]}]
        
        # Fausse réponse de création
        mock_exp_instance.create_experiment.return_value = {"metadata": {"datafiles": ["success"]}}
    ...
    @patch('simple.experiment.pd.read_excel')
    @patch('simple.experiment.silex.ExperimentsApi')
    def test_create_experiment_missing_objective(self, mock_exp_api_class, mock_read_excel):
        ...
        mock_exp_instance = mock_exp_api_class.return_value
        mock_exp_instance.search_experiments.return_value = {"result": []} # L'expérience n'existe pas

        # On vérifie que la fonction crashe bien avec notre erreur personnalisée
        with pytest.raises(DataImportError, match="Objective Missing From MIAPPE file experiment sheet"):
